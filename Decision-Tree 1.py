...
# Call the function plot_boundary from the helper file to get the decision boundaries of both the classifiers
...
# Use the columns above as the features to get the predictor set from the train data
X_train_more = elect_train[pred_cols]

# Use the columns above as the features to get the predictor set from the test data
X_test_more = elect_test[pred_cols]
...
# Fit all the classifiers on the train data
dt1.fit(X_train_more, y_train)
dt2.fit(X_train_more, y_train)
dt3.fit(X_train_more, y_train)


### edTest(test_accuracy) ###
# Compute the train and test accuracy for the first decision tree classifier of depth 2
dt1_train_acc = dt1.score(X_train_more, y_train)
dt1_test_acc = dt1.score(X_test_more, y_test)

# Compute the train and test accuracy for the second decision tree classifier of depth 10
dt2_train_acc = dt2.score(X_train_more, y_train)
dt2_test_acc = dt2.score(X_test_more, y_test)

# Compute the train and test accuracy for the third decision tree classifier of depth 15
dt3_train_acc = dt3.score(X_train_more, y_train)
dt3_test_acc = dt3.score(X_test_more, y_test)
